...
N1 = sym.Matrix(
    [
        sym.Piecewise((1 - ksi, sym.And(xsymi <= xsym, xsym <= xsymip1)), (0, True)),
...
N2 = sym.Matrix(
    [
        sym.Piecewise(
            (1 - 3 * ksi + 2 * ksi**2, sym.And(xsymi <= xsym, xsym <= xsymip1)),
            (0, True),
        ),
        sym.Piecewise(
            (4 * ksi - 4 * ksi**2, sym.And(xsymi <= xsym, xsym <= xsymip1)), (0, True)
        ),
        sym.Piecewise(
            (-ksi + 2 * ksi**2, sym.And(xsymi <= xsym, xsym <= xsymip1)), (0, True)
        ),
    ]
)
...
def buildMat(x, Ne):
    ...
    Me_lmbd = sym.lambdify(
        (xsymi, xsymip1), (Ne @ Ne.T).integrate((xsym, xsymi, xsymip1)), "numpy"
    )
...
def buildf(x, Ne, xs):
    ...
    ndof = len(Ne)  # Number of degrees of freedom per element
    Ndof = (N - 1) * (ndof - 1) + 1  # Number of degrees of freedom TOTAL
    fe_lmbd = sym.lambdify(
        (xsymi, xsymip1), (Ne * f_i).integrate((xsym, xsymi, xsymip1)), "numpy"
    )
    f = np.zeros(Ndof)
...
for nt in tqdm(range(Nt)):
    ...
    ufem_0 = (
        2 * ufem_1
        - ufem_2
        + dt**2 * c2fem * Minv.T @ (f * s[nt] / (dxs[0] * c2fem) - K.T @ ufem_1)
    )
